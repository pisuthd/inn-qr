...
function Repay({ isOpen, onClose, initialMarketId, onSuccess }) {
    ...
    const handleMax = () => {
        // Cap at borrowed amount
        const maxRepay = Math.min(Number(usdcBalance), currentPosition?.borrowed || 0);
        setRepayAmount(formatBalance(maxRepay));
    };

    const handleRepay = async () => {
        if (!initiaAddress || !repayAmount || parseFloat(repayAmount) <= 0 || !currentMarket) return;
        const rawAmount = BigInt(Math.floor(parseFloat(repayAmount) * 1_000_000));
    ...
    return (
        <Modal
            ...
            onClose={onClose}
            title="Repay"
            subtitle="Repay your debt to unlock your collateral"
        >
            {loading ? (
                        ...
                        <div style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(0, 229, 196, 0.15)', borderRadius: '12px', padding: '1.25rem' }}>
                            ...
                            <div style={{ marginBottom: '0.5rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(0,229,196,0.2)', borderRadius: '10px', padding: '0.75rem' }}>
                                    <input type="number" value={repayAmount} onChange={(e) => setRepayAmount(e.target.value)} placeholder="0.0" style={{ flex: 1, background: 'transparent', border: 'none', color: '#ffffff', fontSize: '1rem', outline: 'none' }} />
