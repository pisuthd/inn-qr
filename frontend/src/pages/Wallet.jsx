...
import { RefreshCw, Plus, Wallet as WalletIcon, ArrowLeftRight } from 'lucide-react';
import {
    CHAIN_ID,
    MODULE_ADDRESS, 
    REST_URL,
...
// Token groups by source chain
const TOKEN_GROUPS = [
    {
        label: "Initia L1 ←→ WeaveLink",
        bridgeConfig: { srcChainId: 'initiation-2', srcDenom: 'uinit' },
        tokenIds: [0, 1, 2],
    },
    {
        label: "Cabal ←→ WeaveLink",
        bridgeConfig: { srcChainId: 'cabal-1', srcDenom: 'uiusd' },
        tokenIds: [3, 4],
    },
];
...
function Wallet() {
    ...
    const { initiaAddress, requestTxSync, openConnect, openBridge } = useInterwovenKit();
    ...
    const handleBridge = (bridgeConfig) => {
    ...
    return (
        <div className="fade-in">
            ...
            {TOKEN_GROUPS.map((group) => {
                ...
                return (
                    <div key={group.label} className="card" style={{ marginBottom: '1rem' }}>
                            ...
                            {groupTokens.map((token) => (
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        ...
                                        <div>
                                            <div style={{ color: '#ffffff', fontWeight: '600', fontSize: '0.9rem' }}>{token.name}</div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <span style={{ color: '#b8f5e3', fontSize: '0.9rem', fontWeight: '600' }}>
                        ...
                        {/* Bridge Button */}
                        <button
                            onClick={() => handleBridge(group.bridgeConfig)}
                            className="btn btn-secondary"
            ...
            <div className="card" style={{ background: 'rgba(0, 229, 196, 0.1)' }}>
                <p style={{ color: '#b8f5e3', fontSize: '0.8rem', textAlign: 'center', margin: 0, lineHeight: 1.5 }}>
                    Click + to mint test tokens. Use Bridge to bring real assets from connected chains.
                </p>
            </div>
        </div>
    );
