...
module weavelink::test_market {
    ...
    #[expected_failure(abort_code = market::EUNAUTHORIZED)]
    fun test_create_market_unauthorized(deployer: &signer, user_1: &signer) {
        market::init_module_for_testing(deployer);
        price_oracle::init_for_testing(deployer);

        // user_1 is not the owner, should fail
        market::create_market(user_1, USDC, S_INIT, 80, 1000000, 1000000, 80, 2000000);
    }
    ...
    fun test_cannot_create_market_same_token(deployer: &signer) {
        ...
        market::create_market(deployer, USDC, USDC, 80, 1000000, 1000000, 80, 2000000);
    }
    ...
    fun test_cannot_create_market_lltv_over_100(deployer: &signer) {
        ...
        market::create_market(deployer, USDC, S_INIT, 101, 1000000, 1000000, 80, 2000000);
    }
    ...
    fun test_multiple_markets(deployer: &signer) {
        ...
        market::create_market(deployer, USDC, S_INIT, 80, 1000000, 1000000, 80, 2000000);
        market::create_market(deployer, S_INIT, USDC, 70, 500000, 500000, 80, 1000000);

        assert!(market::get_next_market_id() == 3, 100);

        let (loan1, col1, _, _, _, lltv1, _) = market::get_market(1);
        assert!(loan1 == USDC, 101);
        assert!(col1 == S_INIT, 102);
        assert!(lltv1 == 80, 103);

        let (loan2, col2, _, _, _, lltv2, _) = market::get_market(2);
    ...
    fun test_set_authorization(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::set_authorization(user_1, @0x2222, true);
        assert!(market::is_delegate_authorized(@0x1111, @0x2222), 100);

        // Revoke authorization
        market::set_authorization(user_1, @0x2222, false);
    ...
    fun test_cannot_authorize_self(deployer: &signer, user_1: &signer) {
        ...
        market::set_authorization(user_1, @0x1111, true);
    }
    ...
    fun test_supply_and_withdraw(deployer: &signer, user_1: &signer) {
        ...
        // Supply 1000 USDC
        market::supply(user_1, @0x1111, 1, 1000);
        ...
        // Check market totals
        let (_, _, total_supply, _, _, _, _) = market::get_market(1);
        ...
        // Withdraw 500
        market::withdraw(user_1, @0x1111, 1, 500);

        let (supplied2, _, _) = market::get_position(@0x1111, 1);
    ...
    fun test_supply_on_behalf(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::supply(user_2, @0x1111, 1, 1000);

        // Position belongs to user_1, not user_2
        let (supplied1, _, _) = market::get_position(@0x1111, 1);
        assert!(supplied1 == 1000, 100);

        let (supplied2, _, _) = market::get_position(@0x2222, 1);
    ...
    fun test_cannot_supply_zero(deployer: &signer, user_1: &signer) {
        setup_all(deployer, user_1, user_1);
        market::supply(user_1, @0x1111, 1, 0);
    }
    ...
    fun test_cannot_supply_more_than_balance(deployer: &signer, user_1: &signer) {
        ...
        market::supply(user_1, @0x1111, 1, 15000);
    }
    ...
    fun test_cannot_withdraw_zero(deployer: &signer, user_1: &signer) {
        setup_all(deployer, user_1, user_1);
        market::supply(user_1, @0x1111, 1, 1000);
        market::withdraw(user_1, @0x1111, 1, 0);
    }
    ...
    fun test_cannot_withdraw_more_than_supplied(deployer: &signer, user_1: &signer) {
        setup_all(deployer, user_1, user_1);
        market::supply(user_1, @0x1111, 1, 500);
        market::withdraw(user_1, @0x1111, 1, 1000);
    }
    ...
    fun test_cannot_supply_collateral_more_than_balance(deployer: &signer, user_1: &signer) {
        ...
        market::supply_collateral(user_1, @0x1111, 1, 15000);
    }
    ...
    fun test_cannot_withdraw_collateral_below_health(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::withdraw_collateral(user_1, @0x1111, 1, 600);
    }
    ...
    fun test_borrow_and_repay(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        // Borrow 500 USDC
        market::borrow(user_1, @0x1111, 1, 500);
        ...
        // Repay 200 USDC
        market::repay(user_1, @0x1111, 1, 200);
        assert!(mock_tokens::balance_of(@0x1111, USDC) == 9300, 104); // 9500 - 200

        let (_, borrowed2, _) = market::get_position(@0x1111, 1);
    ...
    fun test_full_repay_borrow(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        // Repay all
        market::repay(user_1, @0x1111, 1, 500);
    ...
    fun test_cannot_borrow_zero(deployer: &signer, user_1: &signer) {
        ...
        market::supply(user_1, @0x1111, 1, 1000);
        market::supply_collateral(user_1, @0x1111, 1, 1000);
        market::borrow(user_1, @0x1111, 1, 0);
    }
    ...
    fun test_cannot_borrow_above_ltv(deployer: &signer, user_1: &signer) {
        ...
        market::borrow(user_1, @0x1111, 1, 900);
    }

    #[test(deployer = @weavelink, user_1 = @0x1111)]
    #[expected_failure(abort_code = market::EINSUFFICIENT_LIQUIDITY)]
    fun test_cannot_borrow_more_than_liquidity(deployer: &signer, user_1: &signer) {
        setup_all(deployer, user_1, user_1);

        // Only 1000 USDC supplied
        market::supply(user_1, @0x1111, 1, 1000);
        market::supply_collateral(user_1, @0x1111, 1, 10000);

        // Try to borrow 2000 when only 1000 available
        market::borrow(user_1, @0x1111, 1, 2000);
    }
    ...
    fun test_repay_more_than_borrowed(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::supply(user_1, @0x1111, 1, 500);
        market::supply_collateral(user_1, @0x1111, 1, 500);
        market::borrow(user_1, @0x1111, 1, 50);

        // User has: 10000 - 500 + 50 = 9550 USDC
        // Repay 100 when only 50 borrowed => should cap at 50
        market::repay(user_1, @0x1111, 1, 100);
    ...
    fun test_health_factor_no_borrow(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        let hf = market::get_health_factor(@0x1111, 1);
        assert!(hf == 100, 100);
    }
    ...
    fun test_health_factor_calculation(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::supply(user_1, @0x1111, 1, 1000);
        market::supply_collateral(user_1, @0x1111, 1, 1000);
        market::borrow(user_1, @0x1111, 1, 100);
    ...
    fun test_liquidation(deployer: &signer, user_1: &signer, user_2: &signer) {
        ...
        market::supply_collateral(user_1, @0x1111, 1, 100);
        // user_1 borrows 80 USDC (max with lltv=80: 100*0.8=80)
        market::borrow(user_1, @0x1111, 1, 80);
        ...
        market::liquidate(user_2, @0x1111, 1, 80);
    ...
    fun test_operations_on_nonexistent_market(deployer: &signer) {
        ...
        let (loan, col, supply, borrow, col_amt, lltv, rate) = market::get_market(999);
        assert!(loan == 0, 100);
        assert!(col == 0, 101);
        assert!(supply == 0, 102);
        assert!(borrow == 0, 103);
        ...
        let (s, b, c) = market::get_position(@0x1111, 999);
        assert!(s == 0, 107);
        assert!(b == 0, 108);
        assert!(c == 0, 109);

        // Health factor for nonexistent market = 0
        let hf = market::get_health_factor(@0x1111, 999);
        assert!(hf == 0, 110);

        // Health factor with no position = 100
        let hf2 = market::get_health_factor(@0x1111, 1);
