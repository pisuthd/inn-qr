...
module weavelink::test_interest {
    ...
    fun test_borrow_rate_zero_utilization(deployer: &signer, user: &signer) {
        ...
        market_v1::supply(user, @0x1111, 1, 10000);

        let (_, _, _, _, _, _, rate) = market_v1::get_market(1);
    ...
    fun test_borrow_rate_below_kink(deployer: &signer, supplier: &signer, borrower: &signer) {
        ...
        market_v1::supply(supplier, @0x1111, 1, 10000);

        // Borrower supplies collateral and borrows 5000 (50% utilization)
        market_v1::supply_collateral(borrower, @0x2222, 1, 10000);
        market_v1::borrow(borrower, @0x2222, 1, 5000);
    ...
    fun test_borrow_rate_at_kink(deployer: &signer, supplier: &signer, borrower: &signer) {
        ...
        market_v1::supply_collateral(borrower, @0x2222, 1, 10000);
        market_v1::borrow(borrower, @0x2222, 1, 8000);
    ...
    fun test_borrow_rate_above_kink(deployer: &signer, supplier: &signer, borrower: &signer) {
        ...
        market_v1::supply_collateral(borrower, @0x2222, 1, 12000);
        market_v1::borrow(borrower, @0x2222, 1, 9000);
    ...
    fun test_rate_decreases_on_repay(deployer: &signer, supplier: &signer, borrower: &signer) {
        ...
        market_v1::borrow(borrower, @0x2222, 1, 8000);
        let (_, _, _, _, _, _, rate1) = market_v1::get_market(1);
        assert!(rate1 == 2000000, 100);

        // Repay 4000 => 40% util => below kink
        market_v1::repay(borrower, @0x2222, 1, 4000);
        // rate = 1000000 + 1000000 * 40 / 80 = 1000000 + 500000 = 1500000
        let (_, _, _, _, _, _, rate2) = market_v1::get_market(1);
    ...
    fun test_rate_changes_with_supply(deployer: &signer, supplier1: &signer, supplier2: &signer, borrower: &signer) {
        ...
        market_v1::supply(supplier1, @0x1111, 1, 5000);

        // Borrower borrows 4000 (80% util)
        market_v1::supply_collateral(borrower, @0x3333, 1, 10000);
        market_v1::borrow(borrower, @0x3333, 1, 4000);
        ...
        market_v1::supply(supplier2, @0x2222, 1, 5000);
    ...
    fun test_low_base_rate_market(deployer: &signer, user: &signer) {
        ...
        market_v1::create_market(deployer, USDC, S_INIT, 80, 0, 500000, 80, 3000000);
    ...
    fun test_market_totals_tracking(deployer: &signer, user: &signer) {
        ...
        // Supply 5000
        market_v1::supply(user, @0x1111, 1, 5000);
        let (_, _, supply1, borrow1, collateral1, _, _) = market_v1::get_market(1);
        ...
        let (_, _, supply2, borrow2, collateral2, _, _) = market_v1::get_market(1);
        assert!(supply2 == 5000, 103);
        assert!(borrow2 == 0, 104);
        assert!(collateral2 == 3000, 105);

        // Borrow 1000
        market_v1::borrow(user, @0x1111, 1, 1000);
        let (_, _, supply3, borrow3, collateral3, _, _) = market_v1::get_market(1);
        assert!(supply3 == 5000, 106);
        assert!(borrow3 == 1000, 107);
        assert!(collateral3 == 3000, 108);

        // Repay 500
        market_v1::repay(user, @0x1111, 1, 500);
        let (_, _, supply4, borrow4, collateral4, _, _) = market_v1::get_market(1);
